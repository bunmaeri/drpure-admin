...
import admin.myhome.report.service.MyReportCustomersService;

@Controller
public class MyReportCustomersController {
	Logger log = Logger.getLogger(this.getClass());

	@Resource(name="myReportCustomersService")
	private MyReportCustomersService myReportCustomersService;
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value="/my_reports/customers/order.dr")
    public ModelAndView reportSalesOrder(Pagemaker pagemaker, CommandMap commandMap) throws Exception {
    	ModelAndView mv = new ModelAndView("/my_report/customers_order");
...
//    	log.debug("date_end>>>>>>>>>>>>>>>>>>>>>>>>"+date_end);
    	...
    	count = myReportCustomersService.totalOrder(commandMap.getMap()); // 레코드 총 갯수 구함
    	...
    	List<Map<String,Object>> list = myReportCustomersService.order(commandMap.getMap());
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value="/my_reports/customers/reward.dr")
    public ModelAndView reportSalesTax(Pagemaker pagemaker, CommandMap commandMap) throws Exception {
    	ModelAndView mv = new ModelAndView("/my_report/customers_reward");
...
//    	log.debug("date_end>>>>>>>>>>>>>>>>>>>>>>>>"+date_end);
    	...
    	int count = 0;
    	pagemaker.setPage(pagemaker.getPage());
    	count = myReportCustomersService.totalRewardPoints(commandMap.getMap()); // 레코드 총 갯수 구함
    	pagemaker.setCount(count); // 페이지 계산

    	commandMap.put("page", (pagemaker.getPage()-1)*pagemaker.getPER());
    	commandMap.put("per_page", pagemaker.getPER());
    	List<Map<String,Object>> list = myReportCustomersService.rewardPoints(commandMap.getMap());
    	mv.addObject("list", list);
    	if(list.size() > 0){
    		mv.addObject("pageMaker", pagemaker); 
    	}
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value="/my_reports/customers/donator.dr")
    public ModelAndView reportSalesShipping(Pagemaker pagemaker, CommandMap commandMap) throws Exception {
    	ModelAndView mv = new ModelAndView("/my_report/customers_donator");
...
//    	log.debug("date_end>>>>>>>>>>>>>>>>>>>>>>>>"+date_end);
    	...
    	int count = 0;
    	pagemaker.setPage(pagemaker.getPage());
    	count = myReportCustomersService.totalDonator(commandMap.getMap()); // 레코드 총 갯수 구함
    	pagemaker.setCount(count); // 페이지 계산

    	commandMap.put("page", (pagemaker.getPage()-1)*pagemaker.getPER());
    	commandMap.put("per_page", pagemaker.getPER());
    	List<Map<String,Object>> list = myReportCustomersService.donator(commandMap.getMap());
    	mv.addObject("list", list);
    	if(list.size() > 0){
    		mv.addObject("pageMaker", pagemaker); 
    	}
