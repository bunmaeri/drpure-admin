...
import admin.report.service.ReportProductsService;

@Controller
public class ReportProductsController {
	Logger log = Logger.getLogger(this.getClass());
	
	@Resource(name="reportProductsService")
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value="/reports/products/viewed.dr")
    public ModelAndView reportProductsViewed(Pagemaker pagemaker, CommandMap commandMap) throws Exception {
    	ModelAndView mv = new ModelAndView("/report/products_viewed");
    	String page = ObjectUtils.null2Value(commandMap.getMap().get("page"),"1");
    	
    	int count = 0;
    	pagemaker.setPage(pagemaker.getPage());
    	count = reportProductsService.totalProductsViewed(); // 레코드 총 갯수 구함
    	pagemaker.setCount(count); // 페이지 계산
    	
    	int product_viewed_total = reportProductsService.totalProductViews(); // 전체 제품의 조회건수 합계 (% 구하기 위해서)
    	...
    	List<Map<String,Object>> list = reportProductsService.productsViewed(commandMap.getMap());
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value="/reports/products/purchased.dr")
    public ModelAndView reportProductsPurchased(Pagemaker pagemaker, CommandMap commandMap) throws Exception {
    	ModelAndView mv = new ModelAndView("/report/products_purchased");
...
//    	log.debug("date_end>>>>>>>>>>>>>>>>>>>>>>>>"+date_end);
    	...
    	int count = 0;
    	pagemaker.setPage(pagemaker.getPage());
    	count = reportProductsService.totalProductsPurchased(commandMap.getMap()); // 레코드 총 갯수 구함
    	pagemaker.setCount(count); // 페이지 계산

    	commandMap.put("page", (pagemaker.getPage()-1)*pagemaker.getPER());
    	commandMap.put("per_page", pagemaker.getPER());
    	List<Map<String,Object>> list = reportProductsService.productsPurchased(commandMap.getMap());
