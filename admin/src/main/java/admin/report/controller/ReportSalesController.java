...
import admin.report.service.ReportSalesService;

@Controller
public class ReportSalesController {
	Logger log = Logger.getLogger(this.getClass());
	
	@Resource(name="reportSalesService")
...
//    	log.debug("date_end>>>>>>>>>>>>>>>>>>>>>>>>"+date_end);
    	...
    	count = reportSalesService.totalSalesOrder(commandMap.getMap()); // 레코드 총 갯수 구함
...
//    	System.err.println((pagemaker.getPage()-1)*pagemaker.getPER());
    	...
    	List<Map<String,Object>> list = reportSalesService.salesOrder(commandMap.getMap());
...
//    	log.debug("date_end>>>>>>>>>>>>>>>>>>>>>>>>"+date_end);
    	...
    	int count = 0;
    	pagemaker.setPage(pagemaker.getPage());
    	count = reportSalesService.totalSalesTax(commandMap.getMap()); // 레코드 총 갯수 구함
    	pagemaker.setCount(count); // 페이지 계산

    	commandMap.put("page", (pagemaker.getPage()-1)*pagemaker.getPER());
    	commandMap.put("per_page", pagemaker.getPER());
    	List<Map<String,Object>> list = reportSalesService.salesTax(commandMap.getMap());
    	mv.addObject("list", list);
    	if(list.size() > 0){
    		mv.addObject("pageMaker", pagemaker); 
    	}
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value="/reports/sales/shipping.dr")
    public ModelAndView reportSalesShipping(Pagemaker pagemaker, CommandMap commandMap) throws Exception {
    	ModelAndView mv = new ModelAndView("/report/sales_shipping");
...
//    	log.debug("date_end>>>>>>>>>>>>>>>>>>>>>>>>"+date_end);
    	...
    	int count = 0;
    	pagemaker.setPage(pagemaker.getPage());
    	count = reportSalesService.totalSalesShipping(commandMap.getMap()); // 레코드 총 갯수 구함
    	pagemaker.setCount(count); // 페이지 계산

    	commandMap.put("page", (pagemaker.getPage()-1)*pagemaker.getPER());
    	commandMap.put("per_page", pagemaker.getPER());
    	List<Map<String,Object>> list = reportSalesService.salesShipping(commandMap.getMap());
    	mv.addObject("list", list);
    	if(list.size() > 0){
    		mv.addObject("pageMaker", pagemaker); 
    	}
