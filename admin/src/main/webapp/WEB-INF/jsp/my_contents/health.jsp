...
<div class="page-content-wrapper">
    <div class="page-content">
        <h1 class="page-title"> <span style="color:red">[My Home Doc]</span> 건강정보 <small>Health Informations</small></h1>
        ${ctag:getSuccess(successMsg)}
        ${ctag:getError(errroMsg)}
        <div class="row">
	        <div class="col-md-12">
	        	<!-- BEGIN FORM-->
	            <form action="/my_contents/security/health.dr" id="form" class="horizontal-form">
	                ...
	                <div class="portlet-body form">
                        ...
                        <div>
                        	<div class="form-actions left" style="float:left">
	                        	<a href="/my_contents/security/health/add.dr" data-toggle="tooltip" title="새로 추가" class="btn green"><i class="fa fa-plus"></i> 추가</a>
	                        </div>
	                    	...
	                    	<div class="table-responsive">
			                    <table class="table table-striped table-bordered table-hover table-checkable" id="datatable_orders">
									...
									<c:forEach items="${list}" var="item">
										<tr role="row">
											<td class="text-align-left"><a href="/my_contents/security/health/contents/${item.contents_id}/${ctag:getLanguage()}.dr">${item.title}</a></td>
...
$(function(){
    $('#filter_name').autocomplete({"source":function(request,response){
       $.getJSON("/my_contents/security/health/autocomplete.dr",{"filter_name":$('#filter_name').val()},
       function(json) {
...
function goPage(page) {
	var action = '/my_contents/security/health.dr';
	$('#page').val(page);
...
function goDelete(contents_id) {
	if(confirm('삭제하시겠습니까?')) {
		var action = '/my_contents/security/health/delete/'+contents_id+'.dr';
		$("#form").attr("action", action);
		$('#form').submit();
	}
}
