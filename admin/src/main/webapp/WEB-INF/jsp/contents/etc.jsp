...
<div class="page-content-wrapper">
    <div class="page-content">
        <h1 class="page-title"> 기타 컨텐츠</h1>
        ${ctag:getSuccess(successMsg)}
        ${ctag:getError(errroMsg)}
        <div class="row">
	        <div class="col-md-12">
	        	<!-- BEGIN FORM-->
	            <form action="/contents/etc.dr" id="form" class="horizontal-form">
	                ...
	                <div class="portlet-body form">
	                    <div>
	                        <div class="form-actions right">
	                        	<a href="/contents/etc/add.dr" data-toggle="tooltip" title="새로 추가" class="btn green"><i class="fa fa-plus"></i> 추가</a>
	                        </div>
                        </div>
	                </div>
	            </div>
	                    	...
	                    	<div class="table-responsive">
			                    <table class="table table-striped table-bordered table-hover table-checkable" id="datatable_orders">
									...
									<c:forEach items="${list}" var="item">
										<tr role="row">
											<td class="text-align-left"><a href="/contents/etc/info/${item.contents_id}/${ctag:getLanguage()}.dr">${item.title}</a></td>
...
function goDelete(contents_id) {
	if(confirm('삭제하시면 사용하는 메뉴에 에러가 발생할 수 있습니다.\n그래도 삭제하시겠습니까?')) {
		var action = '/contents/etc/delete/'+contents_id+'.dr';
		$("#form").attr("action", action);
		$('#form').submit();
	}
}
